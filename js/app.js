...
function createUpiTypeUi(parentElem, decodedText) {
  ...
  addKeyValuePairUi(parentElem, "Payee address", searchParams.get("pa"));
...
let QrResult = function (onCloseCallback) {
  ...
  // let scanResultCustomer = document.getElementById("scan-result-customer");
  // let scanResultTaxnumber = document.getElementById("scan-result-taxnumber");
  // let scanResultTimestamp = document.getElementById("scan-result-timestamp");
  // let scanResultTotalamount = document.getElementById(
  //   "scan-result-totalamount"
  // );
  // let scanResultTaxamount = document.getElementById("scan-result-taxamount");
  ...
  this.__onScanSuccess = function (decodedText, decodedResult, scanType) {
    ...
    scanResultCodeType.innerText = toFriendlyCodeType(
      decodedResult.result.format.formatName
    );
    scanResultText.innerText = decodedText;

    decodeBase64(decodedText);
...
function decodeBase64(decodedText) {
  let scanResultCustomer = document.getElementById("scan-result-customer");
  let scanResultTaxnumber = document.getElementById("scan-result-taxnumber");
  let scanResultTimestamp = document.getElementById("scan-result-timestamp");
  let scanResultTotalamount = document.getElementById(
  ...
  } catch (errorMessageFromDecoding) {
    alert(errorMessageFromDecoding);
  } finally {
    for (; byteIndex < decodedString.length; ) {
      var fieldTag = decodedString
        .substring(byteIndex, byteIndex + 1)
        .charCodeAt(0);
      byteIndex += 1;
      var fieldLength = Number(
        decodedString.substring(byteIndex, byteIndex + 1).charCodeAt(0)
      );
      byteIndex += 1;
      var fieldValue = decodedString.substring(
        byteIndex,
      ...
      fields[fieldTag] = {
        tag: fieldTag,
        value:
          1 == fieldTag ? decodeURIComponent(escape(fieldValue)) : fieldValue,
      };
    }
    for (t in fields) {
      console.log(fields[t].value);
      message += langs.en[t - 1] + ": " + fields[t].value + "\n";
    }
    5 <= fields.length
      ? true // ? alert(message) // replace true with ? alert message to see message
      : alert(
          `incorrect number of fields. ${fields.length} detected (Not <= 5)`
        );
  }

  console.log(`Encoded base: ${base64String} \n 
  Decoded: ${message}`);

  scanResultCustomer.innerText = fields[1].value;
  scanResultTaxnumber.innerText = fields[2].value;
  scanResultTimestamp.innerText = fields[3].value;
  scanResultTotalamount.innerText = fields[4].value;
  scanResultTaxamount.innerText = fields[5].value;
}

QrResult.prototype.onScanSuccess = function (
  decodedText,
  decodedResult,
  scanType
) {
